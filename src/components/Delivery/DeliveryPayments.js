// src/components/Delivery/DeliveryPayments.js
...
const DeliveryPayments = ({ setError, setSuccess, theme }) => {
  const { user } = useAuth();
  ...
  // NUEVO: fecha seleccionada + snapshot del día (no editable para domiciliarios)
  ...
  // Función handleRemoveForm removida ya que no se permite a domiciliarios eliminar pagos
  ...
  const handleSaveAllForms = async () => {
    try {
        ...
        try {
          ...
          try {
            await updateDoc(doc(db, 'payments', editingPaymentId), {
              ...
              units: parseInt(formFields[0].units) || 0,
              amount: parseInt(formFields[0].amount),
              store: formFields[0].store,
              provider: formFields[0].store,
              updatedAt: serverTimestamp(),
              updatedBy: user?.email || ''
            });
            
            console.log("Actualización exitosa usando updateDoc");
          } catch (updateError) {
            console.error("Error con updateDoc, usando enfoque alternativo:", updateError);
            
            // Si falla, creamos un nuevo documento y mantenemos la referencia al original
            await addDoc(collection(db, 'payments'), {
              ...
              provider: formFields[0].store,
              timestamp: serverTimestamp(),
              createdAtLocal: prevPayment?.createdAtLocal || getColombiaLocalDateString(),
              createdBy: prevPayment?.createdBy || user?.email || '',
              updatedAt: serverTimestamp(),
              updatedBy: user?.email || '',
              isReplacement: true,
              replacesId: editingPaymentId
            });
            
            console.log("Creación de reemplazo exitosa usando addDoc");
          }
          
          // Actualizamos el snapshot del día
          const dayToRefresh = prevPayment?.createdAtLocal || getColombiaLocalDateString();
          await refreshPaymentsSnapshotForDate(dayToRefresh);
          
          setSuccess(`Pago actualizado exitosamente. Has modificado el pago "${formFields[0].name}".`);
        } catch (error) {
          console.error("Error completo al actualizar pago:", error);
          throw error; // Re-lanzamos el error para que sea capturado por el try/catch externo
        }
      } else {
        // Creación normal de nuevos pagos
      ...
      setError(`Error al guardar pagos: ${error.message || 'Ha ocurrido un error desconocido'}`);
    }
  };

  // Función de eliminación desactivada para domiciliarios

  // Editar
  const handleEdit = (payment) => {
    // Verificación adicional: el domiciliario solo puede editar sus propios pagos
    if (payment.createdBy && payment.createdBy !== user?.email) {
      setError('No puedes editar pagos creados por otros usuarios.');
      return;
    }
  ...
  const paymentsOfDay = useMemo(() => {
    // Filtramos por día, pero mostramos todos los pagos (incluso los que no creó el usuario)
  ...
  // Total general del día (oculto para domiciliarios)
  const totalOverallExpenses = useMemo(() => {
    // Este cálculo sigue existiendo pero no se muestra en la UI
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-4 sm:py-8">
      {/* Encabezado / fecha (sin mostrar total) */}
      <div className="flex flex-col sm:flex-row items-center justify-between mb-4 sm:mb-8 p-3 sm:p-4 rounded-xl shadow-lg bg-gradient-to-r from-blue-500 to-indigo-600 text-white">
        ...
        <div className="text-lg font-semibold rounded-lg text-sm px-3 py-2 border border-white/30">
          ...
          {/* Nota: no se permite cambiar la fecha a los domiciliarios */}
        </div>
        
        {/* El total ha sido eliminado para domiciliarios */}
      </div>
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSaveAllForms(); }}>
            {formFields.map((field, index) => (
              <div key={index} className={`p-3 sm:p-4 rounded-lg border mb-4 ${theme === 'dark' ? 'border-gray-600 bg-gray-700' : 'border-gray-200 bg-gray-50'}`}>
                ...
                {/* Botón de eliminar pago removido para domiciliarios */}
              </div>
            ))}
